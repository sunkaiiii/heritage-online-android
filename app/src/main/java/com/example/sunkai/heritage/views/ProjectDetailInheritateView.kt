    ...
    fun setData(datas: List<InheritatePeople>) {
        ...
        while (i < datas.size) {
            ...
            val contentLayout: LinearLayout =
                contentView.findViewById(R.id.inheritateMainContentLayout)
            contentView.setOnClickListener {
                findNavController().navigate(
                    R.id.project_detail_to_inheritate_detail,
                    bundleOf(DATA to content.link)
                )
            }
            contentView.findViewById<ImageView>(R.id.inheritateShowMore).setOnClickListener {
                if (context is AppCompatActivity) {
                    TransitionManager.beginDelayedTransition(
                        (context as AppCompatActivity).window.decorView as ViewGroup,
                        ChangeBounds()
                    )
                }
                contentLayout.visibility =
                    if (contentLayout.visibility == View.GONE) View.VISIBLE else View.GONE
            ...
            content.content.filter {
                !(it.key.contains("姓名").or(it.key.contains("类别"))
                    .or(it.key.contains("民族"))).and(it.value.isNotBlank())
            }.forEach {
